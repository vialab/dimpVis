...
Scatterplot.prototype.updateDraggedPoint = function(id,mouseX,mouseY) {
    ...
    this.widget.selectAll(".displayPoints")
	 .attr("cx", function(d) {
	    /** if (d.id == id){
		    var pointLocations = d.nodes;
			var minDist = 1000000;
			var nextView = -1;
			for (var i=0;i<pointLocations.length;i++){
			     if (i != ref.currentView){
				    var distance = Math.abs(pointLocations[i][0] - mouseX);
					if (distance < minDist){
					     minDist = distance;
						 nextView = i;
					}
...
//Finds the larger number of the two given numbers, returns both as an ordered array [smaller,larger]
Scatterplot.prototype.findLarger = function(num1, num2){     
    if (num1 > num2){
	    return [num2,num1];
    }else {
		return [num1,num2];
